...
(defconst sulami/dark-theme 'doom-solarized-dark-high-contrast)
...
(defun sulami/random-uuid ()
  (interactive)
  (let ((uuid (shell-command-to-string "uuidgen | tr '[:upper:]' '[:lower:]' | tee /dev/tty | pbcopy")))
    (message "Generated UUID: %s" (s-trim uuid))))
#+end_src
...
(use-package dumb-jump
  ...
  :config
  (defun sulami/goto-definition ()
    "Universal goto definition."
    (interactive)
    (evil-set-jump)
    (if lsp-mode
        (lsp-find-definition)
...
(use-package lsp-mode
  ...
  (leader-def
    ...
    "l j" #'lsp-find-definition
    "l l" #'lsp
    "l q" #'lsp-disconnect
    "l Q" #'lsp-workspace-restart
    "l r" #'sulami/find-references
    "l R" #'lsp-rename
...
(use-package rustic
  ...
  ;; (remove-hook 'rust-before-save-hook 'rust-before-save-hook)
  ;; (remove-hook 'rust-after-save-hook 'rust-after-save-hook)
  ;; I don't have lsp-modeline.
  :config/el-patch
...
with `lsp-rust-switch-server'."
    (require 'lsp-rust)
    (el-patch-remove (require 'lsp-modeline))
