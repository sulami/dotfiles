...
sub remake () {
	...
	foreach my $win (@wins) {
		...
		if ($win->{'data_level'} == 2) {
			unshift @display, map { my $cpy = $_; $cpy .= '_message'; $cpy } @display;
		}
		$display = (grep { $_ }
...
Irssi::settings_add_str(setc, set 'display_nokey_message', '');
Irssi::settings_add_str(setc, set 'display_key_message', '');
